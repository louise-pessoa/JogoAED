...
static void desenhar_botao(int x, int y, int w, int h, Color cor, const char *texto) {
    ...
    DrawRectangleRoundedLines((Rectangle){x, y, w, h}, 0.4f, 8, 3, WHITE);
    int tam = 22;
...
void tela_ingredientes(Receita *receita) {
    ...
    DrawRectangleRoundedLines((Rectangle){100, 140, 500, 120}, 0.1f, 8, 3, COR_AZUL);
...
void tela_pilha(const char *passo_atual, int num_passo, int total_passos, double tempo) {
    ...
    DrawRectangleRoundedLines((Rectangle){100, 20, 600, 60}, 0.3f, 8, 3, COR_LARANJA);
    ...
    DrawRectangleRoundedLines((Rectangle){30, 320, 220, 80}, 0.2f, 8, 2, (Color){255,150,180,255});
